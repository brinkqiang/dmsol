...
return function()
	...
	local function _profiling_call(funcinfo)
		...
		assert(report)
		-- save the call time
		if not report.calltime then
			report.calltime    = require ("millisecond")
		end
		report.starttime = report.calltime.start()
	...
	local function _profiling_return(funcinfo)
		...
		if report.calltime then
			report.totaltime = report.totaltime + report.calltime.stop(report.starttime)
		end
	end
